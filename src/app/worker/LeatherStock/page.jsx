...
export default function LeatherStockPage() {
  ...
  // Fetch current logged-in user from DB using email
  ...
  const onSubmit = async (data) => {
    ...
    // normalize leather type
    const normalizedType = data.type.trim().toLowerCase() // convert to lowercase

    setLoading(true)
    try {
      await fetch('/api/stock/leather', {
        method: 'POST',
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-amber-50 p-6">
      ...
      >
        ...
        <div className="mb-4">
          ...
          <input
            type="text"
            placeholder="Example: Cow, Goat, Sheep..."
          ...
          <table className="min-w-full text-sm border border-amber-200">
            <thead className="bg-amber-100">
              <tr>
                ...
                <th className="px-3 py-2 border border-amber-200">Type</th>
            ...
            <tbody>
              {stocks.map((stock) => (
                <tr key={stock._id} className="border-t border-amber-200">
                  ...
                  <td className="px-3 py-2 border">{stock.type}</td>
