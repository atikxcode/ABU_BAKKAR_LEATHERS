...
export default function LeatherStockPage() {
  ...
  const fetchStocks = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/stock/leather')
      if (res.ok) {
        ...
        Swal.fire('Error', errorData.error || 'Failed to fetch stock data', 'error')
      }
    } catch (err) {
      console.error('❌ Network Error:', err)
      Swal.fire('Error', 'Network error occurred while fetching data', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const leatherTypes = stocks && Array.isArray(stocks) 
    ? [...new Set(stocks.map((stock) => stock.type).filter(Boolean))].sort()
    : []
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white shadow-lg rounded-xl p-6 border border-amber-200">
          <h2 className="text-xl font-semibold mb-4 text-amber-900">
            {showMyStocks ? 'My Stock Reports' : "Others' Stock Reports"}
          ...
          {currentDisplayStocks.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg">No stock reports found</p>
              <p className="text-gray-400 text-sm mt-2">
                {showMyStocks
                  ? "You haven't submitted any stock reports yet"
                  : 'No other workers have submitted stock reports'}
              </p>
            </div>
          )}
        </div>
